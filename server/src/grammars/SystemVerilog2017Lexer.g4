 ...
 * 
 * This grammar is generated by hdlConvertor/utils/sv/main.py from the PDF with SystemVerilog
 * IEEE1800-2017 standard. The grammar is target language dependent, but mentioned script can
 * generate grammar for multiple target languages (java, python, c++).
 * 
 *
 */
...
KW_ALIAS: 'alias' {this.language_version >= Language.SV2005}?;
KW_ALWAYS: 'always';
KW_ALWAYS_COMB:
...
KW_AND: 'and';
KW_ASSERT: 'assert' {this.language_version >= Language.SV2005}?;
KW_ASSIGN: 'assign';
KW_ASSUME: 'assume' {this.language_version >= Language.SV2005}?;
KW_AUTOMATIC:
	'automatic' {this.language_version >= Language.VERILOG2001}?;
KW_BEFORE: 'before' {this.language_version >= Language.SV2005}?;
KW_BEGIN: 'begin';
KW_BIND: 'bind' {this.language_version >= Language.SV2005}?;
KW_BINS: 'bins' {this.language_version >= Language.SV2005}?;
KW_BINSOF: 'binsof' {this.language_version >= Language.SV2005}?;
KW_BIT: 'bit' {this.language_version >= Language.SV2005}?;
KW_BREAK: 'break' {this.language_version >= Language.SV2005}?;
KW_BUF: 'buf';
KW_BUFIF0: 'bufif0';
KW_BUFIF1: 'bufif1';
KW_BYTE: 'byte' {this.language_version >= Language.SV2005}?;
KW_CASE: 'case';
KW_CASEX: 'casex';
KW_CASEZ: 'casez';
KW_CELL: 'cell' {this.language_version >= Language.VERILOG2001}?;
KW_CHANDLE:
	'chandle' {this.language_version >= Language.SV2005}?;
KW_CHECKER:
	'checker' {this.language_version >= Language.SV2009}?;
KW_CLASS: 'class' {this.language_version >= Language.SV2005}?;
KW_CLOCKING:
	'clocking' {this.language_version >= Language.SV2005}?;
KW_CMOS: 'cmos';
KW_CONFIG:
	'config' {this.language_version >= Language.VERILOG2001}?;
KW_CONST: 'const' {this.language_version >= Language.SV2005}?;
KW_CONSTRAINT:
...
KW_COVER: 'cover' {this.language_version >= Language.SV2005}?;
KW_COVERGROUP:
	'covergroup' {this.language_version >= Language.SV2005}?;
KW_COVERPOINT:
	'coverpoint' {this.language_version >= Language.SV2005}?;
KW_CROSS: 'cross' {this.language_version >= Language.SV2005}?;
...
KW_DISABLE: 'disable';
KW_DIST: 'dist' {this.language_version >= Language.SV2005}?;
KW_DO: 'do' {this.language_version >= Language.SV2005}?;
KW_EDGE: 'edge';
KW_ELSE: 'else';
KW_END: 'end';
KW_ENDCASE: 'endcase';
...
KW_ENDTASK: 'endtask';
KW_ENUM: 'enum' {this.language_version >= Language.SV2005}?;
KW_EVENT: 'event';
KW_EVENTUALLY:
	'eventually' {this.language_version >= Language.SV2009}?;
KW_EXPECT: 'expect' {this.language_version >= Language.SV2005}?;
KW_EXPORT: 'export' {this.language_version >= Language.SV2005}?;
KW_EXTENDS:
	'extends' {this.language_version >= Language.SV2005}?;
KW_EXTERN: 'extern' {this.language_version >= Language.SV2005}?;
KW_FINAL: 'final' {this.language_version >= Language.SV2005}?;
KW_FIRST_MATCH:
...
KW_GLOBAL: 'global' {this.language_version >= Language.SV2009}?;
KW_HIGHZ0: 'highz0';
KW_HIGHZ1: 'highz1';
KW_IF: 'if';
KW_IFF: 'iff' {this.language_version >= Language.SV2005}?;
KW_IFNONE: 'ifnone';
KW_IGNORE_BINS:
...
KW_IMPORT: 'import' {this.language_version >= Language.SV2005}?;
KW_INCDIR:
	'incdir' {this.language_version >= Language.SV2005}?; // used only in sv library file
KW_INCLUDE:
	'include' {this.language_version >= Language.SV2005}?; // used only in sv library file
KW_INITIAL: 'initial';
KW_INOUT: 'inout';
KW_INPUT: 'input';
KW_INSIDE: 'inside' {this.language_version >= Language.SV2005}?;
KW_INSTANCE:
	'instance' {this.language_version >= Language.VERILOG2001}?;
KW_INT: 'int' {this.language_version >= Language.SV2005}?;
KW_INTEGER: 'integer';
KW_INTERCONNECT:
...
KW_LARGE: 'large';
KW_LET: 'let' {this.language_version >= Language.SV2009}?;
KW_LIBRARY:
	'library' {this.language_version >= Language.SV2005}?; // used only in sv library file
KW_LIBLIST:
	'liblist' {this.language_version >= Language.VERILOG2001}?;
KW_LOCAL: 'local' {this.language_version >= Language.SV2005}?;
KW_LOCALPARAM:
	'localparam' {this.language_version >= Language.VERILOG2001}?;
KW_LOGIC: 'logic' {this.language_version >= Language.SV2005}?;
KW_LONGINT:
...
KW_NEW: 'new' {this.language_version >= Language.SV2005}?;
KW_NEXTTIME:
...
KW_NOTIF0: 'notif0';
KW_NOTIF1: 'notif1';
KW_NULL: 'null' {this.language_version >= Language.SV2005}?;
KW_OPTION: 'option';
KW_OR: 'or';
KW_OUTPUT: 'output';
KW_PACKAGE:
	'package' {this.language_version >= Language.SV2005}?;
KW_PACKED: 'packed' {this.language_version >= Language.SV2005}?;
...
KW_PURE: 'pure' {this.language_version >= Language.SV2005}?;
KW_RAND: 'rand' {this.language_version >= Language.SV2005}?;
KW_RANDC: 'randc' {this.language_version >= Language.SV2005}?;
KW_RANDCASE:
...
KW_REF: 'ref' {this.language_version >= Language.SV2005}?;
KW_REG: 'reg';
KW_REJECT_ON:
...
KW_RETURN: 'return' {this.language_version >= Language.SV2005}?;
KW_RNMOS: 'rnmos';
KW_RPMOS: 'rpmos';
KW_RTRAN: 'rtran';
...
KW_SMALL: 'small';
KW_SOFT: 'soft' {this.language_version >= Language.SV2012}?;
KW_SOLVE: 'solve' {this.language_version >= Language.SV2005}?;
KW_SPECIFY: 'specify';
KW_SPECPARAM: 'specparam';
KW_STATIC: 'static' {this.language_version >= Language.SV2005}?;
KW_STD: 'std';
KW_STRING: 'string' {this.language_version >= Language.SV2005}?;
KW_STRONG: 'strong' {this.language_version >= Language.SV2009}?;
KW_STRONG0: 'strong0';
KW_STRONG1: 'strong1';
KW_STRUCT: 'struct' {this.language_version >= Language.SV2005}?;
KW_SUPER: 'super' {this.language_version >= Language.SV2005}?;
KW_SUPPLY0: 'supply0';
KW_SUPPLY1: 'supply1';
KW_SYNC_ACCEPT_ON:
...
KW_TAGGED: 'tagged' {this.language_version >= Language.SV2005}?;
KW_TASK: 'task';
KW_THIS: 'this' {this.language_version >= Language.SV2005}?;
KW_THROUGHOUT:
...
KW_WIRE: 'wire';
KW_WITH: 'with' {this.language_version >= Language.SV2005}?;
KW_WITHIN: 'within' {this.language_version >= Language.SV2005}?;
KW_WOR: 'wor';
KW_XNOR: 'xnor';
KW_XOR: 'xor';
...
UNBASED_UNSIZED_LITERAL: APOSTROPHE Z_OR_X | '\'0' | '\'1';
...
fragment DECIMAL_NUMBER_WITH_BASE: DECIMAL_BASE UNSIGNED_NUMBER;
...
fragment DECIMAL_BASE:
	APOSTROPHE (WHITE_SPACE)? ([sS])? (WHITE_SPACE)? [dD] (
		WHITE_SPACE
	)?;
fragment BINARY_BASE:
	APOSTROPHE (WHITE_SPACE)? ([sS])? (WHITE_SPACE)? [bB] (
		WHITE_SPACE
	)?;
fragment OCTAL_BASE:
	APOSTROPHE (WHITE_SPACE)? ([sS])? (WHITE_SPACE)? [oO] (
		WHITE_SPACE
	)?;
fragment HEX_BASE:
	APOSTROPHE (WHITE_SPACE)? ([sS])? (WHITE_SPACE)? [hH] (
		WHITE_SPACE
	)?;
